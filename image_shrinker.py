...
def resize_image(input_path, output_path):
    try:
        with Image.open(input_path) as img:
            ...
            scaling_factor = 512 / max(img.size)
            if scaling_factor < 1:  # Only resize if the image is larger than 512 on its longest side
                ...
                print(f"Image is smaller than 512 pixels on the longest side, saved original to {output_path}")
    except Exception as e:
        print(f"Error processing {input_path}: {e}")

def process_images(source_folder, target_folder='512Images'):
